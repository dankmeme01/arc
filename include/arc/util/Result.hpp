...
#define ARC_CO_UNWRAP_INTO(v, ...) GEODE_CO_UNWRAP_INTO(v, __VA_ARGS__)
...
#define ARC_MAP_UNWRAP_INTO(dest, ...) GEODE_UNWRAP_INTO(dest, (__VA_ARGS__).mapErr([](auto err) { return err.message(); }))
#define ARC_CO_MAP_UNWRAP_INTO(dest, ...) GEODE_CO_UNWRAP_INTO(dest, (__VA_ARGS__).mapErr([](auto err) { return err.message(); }))
